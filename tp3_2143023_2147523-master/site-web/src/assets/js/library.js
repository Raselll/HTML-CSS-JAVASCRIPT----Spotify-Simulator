...
export class Library {
  ...
  searchInFields (searchFields, searchValue, exactMatch) {
    for (let i = 0; i < searchFields.length; i++) {
      if (this.includesSubstring(searchFields[i], searchValue, exactMatch)) {
        return true;
      }
    }
    return false;
  ...
  search (searchInput, searchSources, exactMatch) {
    const playlists = searchSources.playlists;
    const songs = searchSources.songs;

    const returnedPlaylists = [];
    const returnedSongs = [];

    for (let i = 0; i < playlists.length; i++) {
      const PlaylistItemsArray = [];
      PlaylistItemsArray.push(playlists[i].name);
      PlaylistItemsArray.push(playlists[i].description);
      if (this.searchInFields(PlaylistItemsArray, searchInput.value, exactMatch)) {
        returnedPlaylists.push(playlists[i]);
      }
      // PlaylistItemsArray.splice(0,tempStringArray.length);
    }
    ...
    this.generateLists(returnedPlaylists, returnedSongs);
  }
}
