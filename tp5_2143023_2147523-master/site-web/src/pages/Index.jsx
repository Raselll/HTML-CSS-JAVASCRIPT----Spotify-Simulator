...
export default function Index() {
  ...
  useEffect(() => {
    ...
    api.fetchAllSongs().then((songs) => setSongs(songs)).catch(() => setSongs([]));
  }, [api]);
  ...
  const handleSearch = async (event, query, exactMatch) => {
    ...
    // TODO : implémenter la recherche et la mise à jour de l'interface
    await api.search(query, exactMatch).then((items) => {
      setPlaylists(items.playlists);
      setSongs(items.songs);
    }).catch(() => {
      setPlaylists([]);
      setSongs([]);
    })
