...
class SongService {
  ...
  async updateSongLike(id) {
    ...
    this.collection.findOneAndUpdate({ _id: id }, { $set: { "liked": !(song.liked) } });
    return !(song.liked);
  }
  ...
  async search(substring, exact) {
    ...
    if (exact) {
      ...
      filter = {
        $or: [{ name: { $regex: `${substring}`, $options: "" } },
        { genre: { $regex: `${substring}`, $options: "" } },
        { artist: { $regex: `${substring}`, $options: "" } }
        ]
      };
    }
